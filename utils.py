...
def trim_outer_white(
    ...
    sat = hsv[:, :, 1]
    low_sat = (sat < min_saturation).astype(np.uint8)
    low_sat_border = _flood_fill_border(low_sat)
    content_mask = content_mask & (~low_sat_border)
...
def _flood_fill_border(mask: np.ndarray) -> np.ndarray:
    height, width = mask.shape
    filled = mask.copy()
...
def remove_border_artifacts(
    ...
    for label in range(1, num_labels):
        ...
        thickness = min(w, h)
        if touches_border and (area < min_area or thickness <= max_thickness_px):
